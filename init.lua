...
--  NOTE: C-hjkl are now owned by the vim-tmux-navigator plugin
--  (lua/custom/plugins/tmux-navigator.lua), which moves between nvim
--  splits AND tmux panes seamlessly. See `:help wincmd` for raw window commands.
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      vim.keymap.set('n', '<leader>sG', function()
        builtin.live_grep { additional_args = { '--hidden', '--no-ignore', '--glob', '!.git/' } }
      end, { desc = '[S]earch by [G]rep (incl. hidden/ignored)' })
  ...
  -- LSP Plugins
  {
    ...
    config = function()
      ...
      local servers = {
        clangd = {},
        -- Protobuf language server (provided by the `buf` CLI)
        buf_ls = {},
        -- gopls = {},
        -- pyright = {},
        rust_analyzer = {
      ...
      -- These server keys don't map cleanly to Mason package names, so we
      -- install them explicitly below instead of deriving them from `servers`.
      local skip_auto_install = { pylsp = true, buf_ls = true }
      local ensure_installed = vim.tbl_filter(function(name) return not skip_auto_install[name] end, vim.tbl_keys(servers or {}))
      vim.list_extend(ensure_installed, {
        'lua-language-server', -- Lua Language server
        'stylua',              -- Used to format Lua code
        'python-lsp-server',   -- Python LSP (Mason name for pylsp)
        'buf',                 -- Protobuf LSP (buf_ls) + formatter (buf format)
